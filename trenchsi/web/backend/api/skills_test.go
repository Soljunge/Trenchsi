...
func TestHandleListSkills(t *testing.T) {
	...
	oldBuiltin := os.Getenv("TRENCHLAW_BUILTIN_SKILLS")
	if err := os.Setenv("TRENCHLAW_BUILTIN_SKILLS", builtinRoot); err != nil {
		t.Fatalf("Setenv(TRENCHLAW_BUILTIN_SKILLS) error = %v", err)
	}
	defer func() {
		if oldBuiltin == "" {
			_ = os.Unsetenv("TRENCHLAW_BUILTIN_SKILLS")
		} else {
			_ = os.Setenv("TRENCHLAW_BUILTIN_SKILLS", oldBuiltin)
		}
	}()
