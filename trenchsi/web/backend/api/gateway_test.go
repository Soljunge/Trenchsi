...
import (
	...
	"github.com/sipeed/trenchlaw/pkg/auth"
...
func TestGatewayRestartReturnsErrorStatusWhenReplacementFailsToStart(t *testing.T) {
	...
	invalidBinaryPath := filepath.Join(t.TempDir(), "fake-trenchlaw")
	if err := os.WriteFile(invalidBinaryPath, []byte("#!/bin/sh\n"), 0o644); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
	t.Setenv("TRENCHLAW_BINARY", invalidBinaryPath)
...
func TestFindJameclawBinary_EnvOverride(t *testing.T) {
	// Create a temporary file to act as the mock binary
	tmpDir := t.TempDir()
	mockBinary := filepath.Join(tmpDir, "trenchlaw-mock")
	if err := os.WriteFile(mockBinary, []byte("mock"), 0o755); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}

	t.Setenv("TRENCHLAW_BINARY", mockBinary)

	got := utils.FindJameclawBinary()
	if got != mockBinary {
		t.Errorf("FindJameclawBinary() = %q, want %q", got, mockBinary)
	}
}

func TestFindJameclawBinary_EnvOverride_InvalidPath(t *testing.T) {
	// When TRENCHLAW_BINARY points to a non-existent path, fall through to next strategy
	t.Setenv("TRENCHLAW_BINARY", "/nonexistent/trenchlaw-binary")

	got := utils.FindJameclawBinary()
	// Should not return the invalid path; falls back to "trenchlaw" or another found path
	if got == "/nonexistent/trenchlaw-binary" {
		t.Errorf("FindJameclawBinary() returned invalid env path %q, expected fallback", got)
	}
}
