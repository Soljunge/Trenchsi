...
import (
	...
	"github.com/sipeed/trenchlaw/pkg/memory"
	"github.com/sipeed/trenchlaw/pkg/providers"
	"github.com/sipeed/trenchlaw/pkg/session"
)
...
func TestHandleListSessions_JSONLStorage(t *testing.T) {
	...
	sessionKey := jameSessionPrefix + "history-jsonl"
...
func TestHandleListSessions_TitleUsesTrimmedSummary(t *testing.T) {
	...
	sessionKey := jameSessionPrefix + "summary-title"
	if err := store.AddFullMessage(nil, sessionKey, providers.Message{
		Role:    "user",
...
func TestHandleGetSession_JSONLStorage(t *testing.T) {
	...
	sessionKey := jameSessionPrefix + "detail-jsonl"
...
func TestHandleDeleteSession_JSONLStorage(t *testing.T) {
	...
	sessionKey := jameSessionPrefix + "delete-jsonl"
	if err := store.AddFullMessage(nil, sessionKey, providers.Message{
		Role:    "user",
		Content: "delete me",
	}); err != nil {
...
func TestHandleGetSession_LegacyJSONFallback(t *testing.T) {
	...
	sessionKey := jameSessionPrefix + "legacy-json"
...
func TestHandleSessions_FiltersEmptyJSONLFiles(t *testing.T) {
	...
	dir := sessionsTestDir(t, configPath)
	base := filepath.Join(dir, sanitizeSessionKey(jameSessionPrefix+"empty-jsonl"))
