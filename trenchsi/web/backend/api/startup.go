...
func (h *Handler) handleInstall(w http.ResponseWriter, r *http.Request) {
	...
	if err := utils.RunInstall(h.configPath); err != nil {
		http.Error(w, fmt.Sprintf("Failed to run trenchsi install: %v", err), http.StatusInternalServerError)
		return
	}
	...
	json.NewEncoder(w).Encode(systemActionResponse{
		Status:  "ok",
		Message: "Trenchsi install completed.",
	})
}
...
func (h *Handler) handleUninstall(w http.ResponseWriter, r *http.Request) {
	...
	if err := utils.RunUninstall(h.configPath); err != nil {
		http.Error(w, fmt.Sprintf("Failed to run trenchsi uninstall: %v", err), http.StatusInternalServerError)
		return
	}
	...
	json.NewEncoder(w).Encode(systemActionResponse{
		Status:  "ok",
		Message: "Trenchsi uninstall completed.",
	})
}
