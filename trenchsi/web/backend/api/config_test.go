...
func setupJameEnabledEnv(t *testing.T) (string, func()) {
	...
	oldJameHome := os.Getenv("TRENCHLAW_HOME")

	if err := os.Setenv("HOME", tmp); err != nil {
		t.Fatalf("set HOME: %v", err)
	}
	if err := os.Setenv("TRENCHLAW_HOME", filepath.Join(tmp, ".trenchlaw")); err != nil {
		t.Fatalf("set TRENCHLAW_HOME: %v", err)
	}
	...
	cfg.WithSecurity(&config.SecurityConfig{
		...
		Channels: &config.ChannelsSecurity{
			Jame: &config.JameSecurity{Token: "test-jame-token"},
		},
	})
	...
	cleanup := func() {
		...
		if oldJameHome == "" {
			_ = os.Unsetenv("TRENCHLAW_HOME")
		} else {
			_ = os.Setenv("TRENCHLAW_HOME", oldJameHome)
		}
	}
	return configPath, cleanup
}

func TestHandleUpdateConfig_SucceedsWhenJameTokenInSecurityOnly(t *testing.T) {
	configPath, cleanup := setupJameEnabledEnv(t)
	defer cleanup()
	...
	// PUT request with jame enabled but no token in JSON — token is in .security.yml
	req := httptest.NewRequest(http.MethodPut, "/api/config", bytes.NewBufferString(`{
		"version": 1,
		"agents": {
			"defaults": {
...
func TestHandlePatchConfig_SucceedsWhenJameTokenInSecurityOnly(t *testing.T) {
	configPath, cleanup := setupJameEnabledEnv(t)
	defer cleanup()
	...
	// PATCH request changing an unrelated field — jame token still in .security.yml
	req := httptest.NewRequest(http.MethodPatch, "/api/config", bytes.NewBufferString(`{
		"gateway": {
			"log_level": "info"
		}
