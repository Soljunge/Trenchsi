...
func EnsureOnboarded(configPath string) error {
	...
	cmd := execCommand(FindJameclawBinary(), "onboard")
...
func RunInstall(configPath string) error {
	cmd := execCommand(FindJameclawBinary(), "install")
	cmd.Env = append(os.Environ(), config.EnvConfig+"="+configPath)
	...
	if err != nil {
		...
		return fmt.Errorf("run install: %w: %s", err, trimmed)
	}
...
func RunUninstall(configPath string) error {
	cmd := execCommand(FindJameclawBinary(), "uninstall", "--yes")
	cmd.Env = append(os.Environ(), config.EnvConfig+"="+configPath)
	...
	if err != nil {
		...
		return fmt.Errorf("run uninstall: %w: %s", err, trimmed)
	}
