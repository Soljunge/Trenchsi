// jame-echo-server is a minimal Jame Protocol WebSocket server for testing
// the jame_client channel. It accepts connections, prints received messages
// to stdout, and forwards stdin lines as message.create to all connected clients.
//
// Usage:
//
//	go run ./examples/jame-echo-server -addr :9090 -token secret
//
// Then configure jame_client with url=ws://localhost:9090/ws&token=secret.
package main
...
func (s *server) handleWS(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		switch msg.Type {
		case "ping":
			pong := jameMessage{Type: "pong", ID: msg.ID, Timestamp: time.Now().UnixMilli()}
			conn.WriteJSON(pong)
