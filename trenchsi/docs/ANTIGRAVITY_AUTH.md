...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in TrenchLaw.
...
10. [Creating a New Provider in TrenchLaw](#creating-a-new-provider-in-trenchlaw)
...
const antigravityPlugin = {
  ...
  register(api: TrenchLawPluginApi) {
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<TrenchLawConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.25
- TrenchLaw codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.trenchlaw/auth.json`:
...
## Creating a New Provider in TrenchLaw

TrenchLaw providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/trenchlaw/internal/auth/helpers.go`:
...
trenchlaw auth login --provider your-provider

# List models (for Antigravity)
trenchlaw auth models

# Start the gateway
trenchlaw gateway

# Run an agent with a specific model
trenchlaw agent -m "Hello" --model your-model
```
...
export TRENCHLAW_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export TRENCHLAW_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
- **Source Files:**
  ...
  - `pkg/auth/store.go` - Auth credential storage (`~/.trenchlaw/auth.json`)
  - `pkg/providers/factory.go` - Provider factory and protocol routing
  - `pkg/providers/types.go` - Provider interface definitions
  - `cmd/trenchlaw/internal/auth/helpers.go` - Auth CLI commands
...
### "Token expired"
- Refresh OAuth tokens: `trenchlaw auth login --provider antigravity`
...
- Check auth profile storage: `~/.trenchlaw/auth.json`
- Re-run `trenchlaw auth login --provider antigravity`
