...
4. Enter the Token and EncodingAESKey into the TrenchLaw config file, start the service, then return to the admin console to save (WeCom will send a verification request)
...
User sends a message
  │
  ▼
TrenchLaw immediately returns {finish: false} (Agent starts processing)
  │
  ▼
...
If the Agent takes longer than approximately 30 seconds (WeCom's maximum polling window is 6 minutes), TrenchLaw will:

1. Immediately close the stream and show the user: "⏳ Still working on it. The result will be sent shortly."
...
When `welcome_message` is configured, TrenchLaw will automatically reply with it when a user opens the chat window with the AI Bot (`enter_chat` event). Leave it empty to silently ignore the event.

```json
"welcome_message": "Hello! I'm the TrenchLaw AI assistant. How can I help?"
```
...
- Check TrenchLaw logs to see if a GET request from WeCom was received
