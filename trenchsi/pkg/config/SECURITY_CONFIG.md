...
```bash
cp security.example.yml ~/.trenchlaw/.security.yml
```

### Step 2: Fill in your actual values

Edit `~/.trenchlaw/.security.yml` and replace placeholder values with your actual API keys and tokens.

### Step 3: Update config.json

Replace sensitive values in `~/.trenchlaw/config.json` with `ref:` references:
...
Restart TrenchLaw and verify it loads correctly:
```bash
trenchlaw --version
```
...
2. **Set file permissions**: `chmod 600 ~/.trenchlaw/.security.yml`
...
**For models:**
```bash
export TRENCHLAW_MODEL_LIST_GPT-5.4_API_KEY="sk-from-env"
```

**For channels:**
```bash
export TRENCHLAW_CHANNELS_TELEGRAM_TOKEN="token-from-env"
```

**For web tools:**
```bash
export TRENCHLAW_WEB_BRAVE_API_KEY="key-from-env"
```

Environment variables follow this pattern: `TRENCHLAW_<SECTION>_<KEY1>_<KEY2>_<FIELD>` with dots replaced by underscores and converted to uppercase.
