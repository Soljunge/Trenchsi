...
// DefaultConfig returns the default configuration for TrenchLaw.
func DefaultConfig() *Config {
	// Determine the base path for the workspace.
	// Priority: $TRENCHLAW_HOME > detected existing home > ~/.trenchsi
	var homePath string
	if trenchlawHome := os.Getenv(EnvHome); trenchlawHome != "" {
		homePath = trenchlawHome
	} else {
		userHome, _ := os.UserHomeDir()
		homePath = pkg.ResolveDefaultHome(userHome)
	}
	...
	return &Config{
		...
		Trading: TradingConfig{
			...
			Network:                   "mainnet",
			MaxPositionSOL:            0.10,
			MaxDailyLossSOL:           1.00,
			MaxOpenPositions:          1,
			DefaultSlippageBps:        100,
			TakeProfitPct:             20,
			StopLossPct:               10,
			TrailingStopPct:           8,
			TradeCooldownSeconds:      900,
			MinLiquidityUSD:           25000,
			MinVolumeUSD:              10000,
			MinMarketCapUSD:           50000,
			MaxMarketCapUSD:           500000000,
			MinOrganicScore:           25,
			MaxPriceImpactPct:         5,
			MaxVolatilityPct:          40,
			MaxHolderConcentrationPct: 35,
			ReserveSOL:                0.25,
		...
		ModelList: []*ModelConfig{
			...
			// OpenRouter Claude Sonnet - https://openrouter.ai/keys
			{
				ModelName: "openrouter-claude-sonnet-4.6",
