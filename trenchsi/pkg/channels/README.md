# TrenchLaw Channel System: Complete Development Guide

> **Scope**: `pkg/channels/`, `pkg/bus/`, `pkg/media/`, `pkg/identity/`, `cmd/trenchlaw/internal/gateway/`
...
│   ├── init.go
│   └── discord.go
├── slack/ line/ onebot/ dingtalk/ feishu/ wecom/ qq/ whatsapp/ whatsapp_native/ maixcam/ jame/
│   └── ...
...
import (
    "github.com/sipeed/trenchlaw/pkg/bus"
    "github.com/sipeed/trenchlaw/pkg/channels"     // Reference parent package
    "github.com/sipeed/trenchlaw/pkg/config"
    "github.com/sipeed/trenchlaw/pkg/identity"      // New
    "github.com/sipeed/trenchlaw/pkg/media"          // New (if media support needed)
)
```
...
```go
// cmd/trenchlaw/internal/gateway/helpers.go
import (
    _ "github.com/sipeed/trenchlaw/pkg/channels/telegram"   // Triggers init() registration
    _ "github.com/sipeed/trenchlaw/pkg/channels/discord"
    _ "github.com/sipeed/trenchlaw/pkg/channels/your_new_channel"  // New addition
)
```
...
**This means**:
- Channels implementing `TypingCapable` (Telegram, Discord, LINE, Jame) do not need to manually call `StartTyping` + `RecordTypingStop` in `handleMessage`
...
- Channels implementing `PlaceholderCapable` (Telegram, Discord, Jame) do not need to manually send placeholder messages and call `RecordPlaceholder` in `handleMessage`
...
// cmd/trenchlaw/internal/gateway/helpers.go
import (
    _ "github.com/sipeed/trenchlaw/pkg/channels/matrix"
)
```
...
| `"alice"` | Matches Telegram `sender.Username` |
...
4. **Jame Protocol**: `pkg/channels/jame/` implements a custom TrenchLaw native protocol channel that receives messages via WebSocket webhook (`/jame/ws`).
...
7. **PlaceholderConfig vs implementation**: `PlaceholderConfig` appears in 6 channel configs (Telegram, Discord, Slack, LINE, OneBot, Jame), but only channels that implement both `PlaceholderCapable` + `MessageEditor` (Telegram, Discord, Jame) can actually use placeholder message editing. The rest are reserved fields.

8. **ReasoningChannelID**: Most channel configs include a `reasoning_channel_id` field to route LLM reasoning/thinking output to a designated channel (WhatsApp, Telegram, Feishu, Discord, MaixCam, QQ, DingTalk, Slack, LINE, OneBot, WeCom, WeComApp). Note: `JameConfig` does not currently expose this field. `BaseChannel` exposes this via the `WithReasoningChannelID` option and `ReasoningChannelID()` method.
