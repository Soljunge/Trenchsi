...
import (
	...
	"github.com/sipeed/trenchlaw/pkg/migrate/internal"
)

// OpenclawHomeEnvVar is the environment variable that overrides the source
// openclaw home directory when migrating from openclaw to trenchlaw.
// Default: ~/.openclaw
...
func (o *OpenclawHandler) ExecuteConfigMigration(srcConfigPath, dstConfigPath string) error {
	...
	jameCfg, warnings, err := openclawCfg.ConvertToTrenchLaw(o.opts.SourceHome)
	if err != nil {
		return err
	}
	...
	incoming := jameCfg.ToStandardConfig()
...
func rewriteWorkspacePath(path string) string {
	path = strings.Replace(path, ".openclaw", ".trenchlaw", 1)
	return path
}
