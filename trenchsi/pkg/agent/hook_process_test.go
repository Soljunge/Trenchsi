...
func TestProcessHook_HelperProcess(t *testing.T) {
	if os.Getenv("TRENCHLAW_HOOK_HELPER") != "1" {
		return
	}
...
func processHookHelperEnv(mode, eventLog string) []string {
	env := []string{
		"TRENCHLAW_HOOK_HELPER=1",
		"TRENCHLAW_HOOK_MODE=" + mode,
	}
	if eventLog != "" {
		env = append(env, "TRENCHLAW_HOOK_EVENT_LOG="+eventLog)
	}
	return env
}
...
func runProcessHookHelper() error {
	mode := os.Getenv("TRENCHLAW_HOOK_MODE")
	eventLog := os.Getenv("TRENCHLAW_HOOK_EVENT_LOG")
