...
func startGateway() error {
	...
	if runtime.GOOS == "windows" {
		cmd = exec.Command("cmd", "/C", "start /B trenchsi gateway > NUL 2>&1")
	} else {
		cmd = exec.Command("sh", "-c", "nohup trenchsi gateway > /dev/null 2>&1 & echo $! > "+pidPath)
	}
	...
	if runtime.GOOS == "windows" {
		cmd := exec.Command(
			...
			"process",
			"where",
			"name='trenchsi.exe' and commandline like '%gateway%'",
			"get",
			"processid",
		)
		output, err := cmd.Output()
