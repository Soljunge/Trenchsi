...
to authorize your account. On success, the bot token is saved to the trenchsi
config so you can start the gateway immediately.

Example:
  trenchsi auth weixin`,
...
func printManualWeixinConfig(token, baseURL string) {
	fmt.Println()
	fmt.Println("Add the following to the channels section of your trenchsi config:")
	fmt.Println()
	fmt.Println(`  "weixin": {`)
