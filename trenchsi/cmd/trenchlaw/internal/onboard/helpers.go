...
const (
	...
	agentNameLinePrefix        = "Your name is Trenchsi"
)
...
func renderOnboardIntro() {
	...
	onboardWriteLine("%sTrenchsi Onboard%s", onboardANSITitle, onboardANSIReset)
...
func renderOnboardSummary(configExists, encrypt bool, configPath, workspace string, selection onboardSelection) {
	...
	if selection.modelConfigured {
		onboardWriteLine("%sSetup complete. The next time you run `trenchsi`, you can choose terminal agent, dashboard, or web console.%s", onboardANSIDim, onboardANSIReset)
	} else {
		onboardWriteLine("%sSetup is saved, but chat is not ready yet. Run `trenchsi onboard` again to finish model setup.%s", onboardANSIDim, onboardANSIReset)
	}
	onboardWriteLine("")
	...
	startCopy := "Run `trenchsi onboard` again after you choose a model."
	startMarker := "◇"
	startColor := onboardANSIInactive
	if selection.modelConfigured {
		startCopy = "Run `trenchsi` and choose terminal agent, web console, or dashboard."
		startMarker = "◆"
		startColor = onboardANSIActive
	}
	renderOnboardStep(startMarker, startColor, "Start Chat", startCopy)
	onboardWriteLine("  %s│%s Optional web console: trenchsi-web", onboardANSIRail, onboardANSIReset)
	onboardWriteLine("  %s│%s Optional TUI dashboard: trenchsi-launcher-tui", onboardANSIRail, onboardANSIReset)
	onboardWriteLine("  %s│%s", onboardANSIRail, onboardANSIReset)
	onboardWriteLine("")
	nextCommand := "trenchsi onboard"
	if selection.modelConfigured {
		nextCommand = "trenchsi"
	}
...
func renderOnboardWizard(configExists, encrypt bool, configPath, workspace string, selection onboardSelection) {
	...
	modelDescription := "Choose the default model used by `trenchsi`."
	if selection.modelConfigured {
