...
import (
	...
	"github.com/sipeed/trenchlaw/pkg/migrate"
)
...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to trenchlaw",
		Args:  cobra.NoArgs,
		Example: `  trenchlaw migrate
  trenchlaw migrate --from openclaw
  trenchlaw migrate --dry-run
  trenchlaw migrate --refresh
  trenchlaw migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.trenchlaw)")
