...
| allow_from | array  | No       | Allowlist of Telegram user IDs or usernames; empty means all users are allowed |
...
5. (Optional) Configure `allow_from` to restrict who can interact. You can use numeric user IDs from `@userinfobot`, usernames like `@alice`, or bare Telegram usernames like `alice`.

## Built-in Commands

Telegram auto-registers TrenchLaw's top-level bot commands at startup, including `/start`, `/help`, `/show`, `/list`, and `/use`.
