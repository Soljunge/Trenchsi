...
Talk to your trenchlaw through Telegram, Discord, WhatsApp, Matrix, QQ, DingTalk, LINE, WeCom, Feishu, Slack, IRC, OneBot, MaixCam, or Jame (native protocol)
...
| **Jame**             | ⭐ Easy            | Native TrenchLaw protocol channel                      |                                                                                                                  |
...
> You can use a numeric Telegram user ID from `@userinfobot`, `@username`, or a bare Telegram username.
...
TrenchLaw now keeps command definitions in one shared registry. On startup, Telegram will automatically register supported bot commands (for example `/start`, `/help`, `/show`, `/list`, `/use`) so command menu and runtime behavior stay in sync.
...
If command registration fails (network/API transient errors), the channel still starts and TrenchLaw retries registration in the background.
...
TrenchLaw can connect to WhatsApp in two ways:
...
If `session_store_path` is empty, the session is stored in `<workspace>/whatsapp/`. Run `trenchlaw gateway`; on first run, scan the QR code printed in the terminal with WhatsApp → Linked Devices.
...
TrenchLaw supports connecting to your personal WeChat account using the official Tencent iLink API.

**1. Login**

Run the interactive QR login flow:
```bash
trenchlaw auth weixin
```
...
QQ Open Platform provides a one-click setup page for TrenchLaw QQ bots:
...
4. Run `trenchlaw gateway` and open QQ to chat with your bot
...
* Configure as shown above and run `trenchlaw gateway`
...
TrenchLaw supports three types of WeCom integration:
...
TrenchLaw connects to Feishu via WebSocket/SDK mode — no public webhook URL or callback server needed.
...
OneBot is an open protocol for QQ bots. TrenchLaw connects to any OneBot v11 compatible implementation (e.g., [Lagrange](https://github.com/LagrangeDev/Lagrange.Core), [NapCat](https://github.com/NapNeko/NapCatQQ)) via WebSocket.
