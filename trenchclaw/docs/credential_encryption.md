...
TrenchLaw supports encrypting `api_key` values in `model_list` configuration entries.
...
```bash
export TRENCHLAW_KEY_PASSPHRASE="your-passphrase"
```

**2. Encrypt an API key**

Run `trenchlaw onboard` — it prompts for your passphrase and generates the SSH key,
...
| Encrypted | `enc://<base64>` | Decrypted at startup using `TRENCHLAW_KEY_PASSPHRASE` |
...
aes_key = HKDF-SHA256(ikm, salt, info="trenchlaw-credential-v1", 32 bytes)
```
...
1. The **passphrase** (`TRENCHLAW_KEY_PASSPHRASE`)
...
|----------|----------|-------------|
| `TRENCHLAW_KEY_PASSPHRASE` | Yes (for `enc://`) | Passphrase used for key derivation |
| `TRENCHLAW_SSH_KEY_PATH` | No | Path to SSH private key. If not set, auto-detects from `~/.ssh/trenchlaw_ed25519.key` |

### SSH Key Auto-Detection

If `TRENCHLAW_SSH_KEY_PATH` is not set, TrenchLaw looks for the trenchlaw-specific key:

```
~/.ssh/trenchlaw_ed25519.key
```

This dedicated file avoids conflicts with the user's existing SSH keys.
Run `trenchlaw onboard` to generate it automatically.

`os.UserHomeDir()` is used for cross-platform home directory resolution (reads `USERPROFILE` on Windows, `HOME` on Unix/macOS).

> **Note:** An SSH key file is required for credential encryption. If no key is found and `TRENCHLAW_SSH_KEY_PATH` is not set, encryption/decryption will fail. Run `trenchlaw onboard` to generate the key automatically.

---

## Migration

Because the only secret material is `TRENCHLAW_KEY_PASSPHRASE` and the SSH private key file, migration is straightforward:

1. Copy the config file to the new machine.
2. Set `TRENCHLAW_KEY_PASSPHRASE` to the same value.
3. Copy the SSH private key file to the same path (or set `TRENCHLAW_SSH_KEY_PATH` to its new location).
...
- **Both passphrase and SSH key are required.** The SSH key acts as a second factor — without it, encryption/decryption will fail. Run `trenchlaw onboard` to generate the key if it doesn't exist.
- **The SSH key is read-only at runtime.** TrenchLaw never writes to or modifies the SSH key file.
- **Plaintext keys remain supported.** Existing configs without `enc://` are unaffected.
- **The `enc://` format is versioned** via the HKDF `info` field (`trenchlaw-credential-v1`), allowing future algorithm upgrades without breaking existing encrypted values.
