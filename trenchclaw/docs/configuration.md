...
Config file: `~/.trenchlaw/config.json`

### Environment Variables

You can override default paths using environment variables. This is useful for portable installations, containerized deployments, or running trenchlaw as a system service. These variables are independent and control different paths.

| Variable          | Description                                                                                                                             | Default Path              |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `TRENCHLAW_CONFIG` | Overrides the path to the configuration file. This directly tells trenchlaw which `config.json` to load, ignoring all other locations. | `~/.trenchlaw/config.json` |
| `TRENCHLAW_HOME`   | Overrides the root directory for trenchlaw data. This changes the default location of the `workspace` and other data directories.          | `~/.trenchlaw`             |

**Examples:**

```bash
# Run trenchlaw using a specific config file
# The workspace path will be read from within that config file
TRENCHLAW_CONFIG=/etc/trenchlaw/production.json trenchlaw gateway

# Run trenchlaw with all its data stored in /opt/trenchlaw
# Config will be loaded from the default ~/.trenchlaw/config.json
# Workspace will be created at /opt/trenchlaw/workspace
TRENCHLAW_HOME=/opt/trenchlaw trenchlaw agent

# Use both for a fully customized setup
TRENCHLAW_HOME=/srv/trenchlaw TRENCHLAW_CONFIG=/srv/trenchlaw/main.json trenchlaw gateway
```

### Workspace Layout

TrenchLaw stores data in your configured workspace (default: `~/.trenchlaw/workspace`):
...
1. `~/.trenchlaw/workspace/skills` (workspace)
2. `~/.trenchlaw/skills` (global)
...
```bash
export TRENCHLAW_BUILTIN_SKILLS=/path/to/skills
```
...
When multiple bindings exist, TrenchLaw resolves in this order:
...
If a binding points to a missing `agent_id`, TrenchLaw falls back to the default agent.

#### How matching works (step-by-step)

1. TrenchLaw first filters bindings by `match.channel` (must equal current channel).
...
TrenchLaw runs in a sandboxed environment by default. The agent can only access files and execute commands within the configured workspace.
...
| ----------------------- | ----------------------- | ----------------------------------------- |
| `workspace`             | `~/.trenchlaw/workspace` | Working directory for the agent           |
...
The exec safety guard only inspects the command line TrenchLaw launches directly. It does not recursively inspect child
...
* Run TrenchLaw inside a container or VM if you need stronger isolation than the built-in guard provides.
...
```bash
export TRENCHLAW_AGENTS_DEFAULTS_RESTRICT_TO_WORKSPACE=false
```
...
TrenchLaw can perform periodic tasks automatically. Create a `HEARTBEAT.md` file in your workspace:
...
* `TRENCHLAW_HEARTBEAT_ENABLED=false` to disable
* `TRENCHLAW_HEARTBEAT_INTERVAL=60` to change interval
...
| `volcengine` | LLM (Volcengine direct)                 | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=TrenchLaw&utm_content=TrenchLaw&utm_medium=devrel&utm_source=OWO&utm_term=TrenchLaw) |
...
> **What's New?** TrenchLaw now uses a **model-centric** configuration approach. Simply specify `vendor/model` format (e.g., `zhipu/glm-4.7`) to add new providers — **zero code changes required!**
...
| **VolcEngine (Doubao)** | `volcengine/`     | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [Get Key](https://www.volcengine.com/activity/codingplan?utm_campaign=TrenchLaw&utm_content=TrenchLaw&utm_medium=devrel&utm_source=OWO&utm_term=TrenchLaw) |
...
> Run `trenchlaw auth login --provider anthropic` to paste your API token.
...
TrenchLaw strips only the outer `litellm/` prefix before sending the request, so `litellm/lite-gpt4` sends `lite-gpt4`, while `litellm/openai/gpt-4o` sends `openai/gpt-4o`.

</details>

#### Load Balancing

Configure multiple endpoints for the same model name — TrenchLaw will automatically round-robin between them:
...
TrenchLaw routes providers by protocol family:
...
TrenchLaw supports cron-style scheduled tasks via the `cron` tool. The agent can set, list, and cancel reminders or recurring jobs that trigger at specified times.
...
Scheduled tasks persist across restarts and are stored in `~/.trenchlaw/workspace/cron/`.
