...
TrenchLaw's tools configuration is located in the `tools` field of `config.json`.
...
Before tool results are sent to the LLM, TrenchLaw can filter sensitive values (API keys, tokens, secrets) from the output. This prevents the LLM from seeing its own credentials.
...
```bash
TRENCHLAW_TOOLS_EXEC_ENABLED=false
```
...
By default, TrenchLaw blocks the following dangerous commands:
...
The exec guard only validates the top-level command sent to TrenchLaw. It does **not** recursively inspect child
...
All configuration options can be overridden via environment variables with the format `TRENCHLAW_TOOLS_<SECTION>_<KEY>`:

For example:

- `TRENCHLAW_TOOLS_WEB_BRAVE_ENABLED=true`
- `TRENCHLAW_TOOLS_EXEC_ENABLED=false`
- `TRENCHLAW_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `TRENCHLAW_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
- `TRENCHLAW_TOOLS_MCP_ENABLED=true`
