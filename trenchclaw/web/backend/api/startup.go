...
const (
	autoStartEntryName = "TrenchLawLauncher"
	launchAgentLabel   = "io.trenchlaw.launcher"
)
...
func (h *Handler) registerStartupRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("POST /api/system/install", h.handleInstall)
	mux.HandleFunc("POST /api/system/uninstall", h.handleUninstall)
}
...
func (h *Handler) handleInstall(w http.ResponseWriter, r *http.Request) {
	if err := h.StopGatewayForSystemAction(); err != nil {
		http.Error(w, fmt.Sprintf("Failed to stop gateway before install: %v", err), http.StatusInternalServerError)
		return
	}

	if err := utils.RunInstall(h.configPath); err != nil {
		http.Error(w, fmt.Sprintf("Failed to run trenchclaw install: %v", err), http.StatusInternalServerError)
		return
	}

	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(systemActionResponse{
		Status:  "ok",
		Message: "TrenchClaw install completed.",
	})
}

func (h *Handler) handleUninstall(w http.ResponseWriter, r *http.Request) {
	if err := h.StopGatewayForSystemAction(); err != nil {
		http.Error(w, fmt.Sprintf("Failed to stop gateway before uninstall: %v", err), http.StatusInternalServerError)
		return
	}

	if err := utils.RunUninstall(h.configPath); err != nil {
		http.Error(w, fmt.Sprintf("Failed to run trenchclaw uninstall: %v", err), http.StatusInternalServerError)
		return
	}
	...
	json.NewEncoder(w).Encode(systemActionResponse{
		Status:  "ok",
		Message: "TrenchClaw uninstall completed.",
	})
}
...
func linuxAutoStartPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "autostart", "trenchlaw-web.desktop")
}
...
func setLinuxAutoStart(enabled bool, exePath string, args []string) error {
	...
	if enabled {
		...
		content := strings.Join([]string{
			...
			"Version=1.0",
			"Name=TrenchLaw Web",
			"Comment=Start TrenchLaw Web on login",
