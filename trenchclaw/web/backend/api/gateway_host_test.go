...
func TestBuildWsURLUsesRequestHostWhenLauncherPublicSaved(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "http://launcher.local/api/jame/token", nil)
	req.Host = "192.168.1.9:18800"

	if got := h.buildWsURL(req, cfg); got != "ws://192.168.1.9:18800/jame/ws" {
		t.Fatalf("buildWsURL() = %q, want %q", got, "ws://192.168.1.9:18800/jame/ws")
	}
}
...
func TestBuildWsURLUsesWSSWhenForwardedProtoIsHTTPS(t *testing.T) {
	...
	if got := h.buildWsURL(req, cfg); got != "wss://chat.example.com:18800/jame/ws" {
		t.Fatalf("buildWsURL() = %q, want %q", got, "wss://chat.example.com:18800/jame/ws")
	}
}
...
func TestBuildWsURLUsesWSSWhenRequestIsTLS(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "https://launcher.local/api/jame/token", nil)
	req.Host = "secure.example.com"
	req.TLS = &tls.ConnectionState{}

	if got := h.buildWsURL(req, cfg); got != "wss://secure.example.com:18800/jame/ws" {
		t.Fatalf("buildWsURL() = %q, want %q", got, "wss://secure.example.com:18800/jame/ws")
	}
}
...
func TestBuildWsURLPrefersForwardedHTTPOverTLS(t *testing.T) {
	...
	if got := h.buildWsURL(req, cfg); got != "ws://chat.example.com:18800/jame/ws" {
		t.Fatalf("buildWsURL() = %q, want %q", got, "ws://chat.example.com:18800/jame/ws")
	}
}
