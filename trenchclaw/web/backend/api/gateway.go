...
// StopGatewayForSystemAction stops any managed gateway process, including
// attached instances, before install or uninstall actions.
func (h *Handler) StopGatewayForSystemAction() error {
	gateway.mu.Lock()
	defer gateway.mu.Unlock()
...
func (h *Handler) startGatewayLocked(initialStatus string, existingPid int) (int, error) {
	...
	// Start new process
	// Locate the trenchlaw executable
	execPath := utils.FindJameclawBinary()
	...
	// Ensure Jame Channel is configured before starting gateway
	if _, err := h.ensureJameChannel(""); err != nil {
		logger.ErrorC("gateway", fmt.Sprintf("Warning: failed to ensure jame channel: %v", err))
		// Non-fatal: gateway can still start without jame channel
	}
	...
	pid = cmd.Process.Pid
	logger.InfoC("gateway", fmt.Sprintf("Started trenchlaw gateway (PID: %d) from %s", pid, execPath))
...
// handleGatewayStart starts the trenchlaw gateway subprocess.
//
