...
import { type JameMessage, handleJameMessage } from "@/features/chat/protocol"
import {
...
export async function connectChat() {
  ...
  try {
    ...
    socket.onmessage = (event) => {
      ...
      try {
        const message = JSON.parse(event.data) as JameMessage
        handleJameMessage(message, sessionId)
      } catch {
        console.warn("Non-JSON message from jame:", event.data)
      }
    }
