...
func NewUninstallCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:   "Remove local trenchclaw data and reset first-run setup",
		Args:    cobra.NoArgs,
...
func run(opts options) error {
	...
	if !opts.yes {
		...
		if !confirmed {
			fmt.Fprintln(uninstallOutput, "TrenchClaw uninstall canceled.")
			return nil
		}
	}
	...
	if removedCount == 0 {
		fmt.Fprintln(uninstallOutput, "No local TrenchClaw state was found.")
		fmt.Fprintln(uninstallOutput, "The next setup can start with `trenchclaw onboard`.")
		return nil
	}

	fmt.Fprintln(uninstallOutput, "TrenchClaw local state removed.")
	fmt.Fprintln(uninstallOutput, "To start fresh again, run `trenchclaw onboard`.")
	return nil
}
