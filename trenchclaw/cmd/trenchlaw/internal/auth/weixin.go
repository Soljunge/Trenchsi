...
import (
	...
	"github.com/sipeed/trenchlaw/pkg/channels/weixin"
...
to authorize your account. On success, the bot token is saved to the trenchclaw
config so you can start the gateway immediately.

Example:
  trenchclaw auth weixin`,
...
func runWeixinOnboard(baseURL, proxy string, timeout time.Duration) error {
	...
	fmt.Println()
	fmt.Println("  trenchclaw gateway")
	fmt.Println()
...
func printManualWeixinConfig(token, baseURL string) {
	fmt.Println()
	fmt.Println("Add the following to the channels section of your trenchclaw config:")
	fmt.Println()
	fmt.Println(`  "weixin": {`)
