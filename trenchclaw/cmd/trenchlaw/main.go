// TrenchClaw - Ultra-lightweight personal AI agent
// Inspired by and based on nanobot: https://github.com/HKUDS/nanobot
// License: MIT
//
// Copyright (c) 2026 TrenchClaw contributors
...
func renderSetupRequired() {
	...
	fmt.Fprintf(defaultCommandOutput, "%sTrenchClaw is not installed yet.%s\n", startupANSITitle, startupANSIReset)
	fmt.Fprintf(defaultCommandOutput, "%sRun the setup command first, then come back to `trenchclaw`.%s\n", startupANSIDim, startupANSIReset)
	fmt.Fprintln(defaultCommandOutput)
	fmt.Fprintf(defaultCommandOutput, "%sOnboard:%s trenchclaw onboard\n", startupANSIOption, startupANSIReset)
	fmt.Fprintf(defaultCommandOutput, "%sAlias:%s trenchclaw install\n", startupANSIOption, startupANSIReset)
}
...
func runAgentChatCommand() error {
	...
	if cfg.Agents.Defaults.ModelName == "" {
		...
		fmt.Fprintln(defaultCommandOutput)
		fmt.Fprintf(defaultCommandOutput, "%sTry next:%s trenchclaw onboard\n", startupANSIOption, startupANSIReset)
		fmt.Fprintf(defaultCommandOutput, "%sOr set a model:%s trenchclaw model <model-name>\n", startupANSIOption, startupANSIReset)
		return nil
	}
...
func promptStartupChoiceFallback() (string, error) {
	fmt.Fprintln(defaultCommandOutput, "Choose how to start TrenchClaw:")
...
func renderStartupChoice(selected int) {
	fmt.Fprint(defaultCommandOutput, startupANSIBgClear)
	startupWriteLine("%sTrenchClaw Startup%s", startupANSITitle, startupANSIReset)
...
func runTUICommand() error {
	binary, err := resolveLauncherBinary("TRENCHCLAW_TUI_BINARY", "trenchclaw-launcher-tui", "trenchlaw-launcher-tui")
	if err != nil {
		return fmt.Errorf("%w. Build it with `go build -o /Users/horatiubudai/.local/bin/trenchclaw-launcher-tui ./cmd/trenchlaw-launcher-tui`", err)
	}
...
func runWebCommand() error {
	binary, err := resolveLauncherBinary("TRENCHCLAW_WEB_BINARY", "trenchclaw-web", "trenchlaw-web")
	if err != nil {
		return fmt.Errorf("%w. Build it with `CGO_ENABLED=0 go build -o /Users/horatiubudai/.local/bin/trenchclaw-web ./web/backend`", err)
	}
...
func resolveLauncherBinary(envVar string, binaryNames ...string) (string, error) {
	for _, candidateEnv := range []string{envVar, strings.ReplaceAll(envVar, "TRENCHCLAW", "TRENCHLAW")} {
