...
// Package config provides types and I/O for ~/.trenchlaw/tui.toml.
package config
...
func DefaultConfigPath() string {
	...
	return filepath.Join(home, ".trenchlaw", "tui.toml")
}

// TUIConfig is the top-level structure of ~/.trenchlaw/tui.toml.
...
// SyncSelectedModelToMainConfig syncs the currently selected model to ~/.trenchlaw/config.json
...
func SyncSelectedModelToMainConfig(scheme Scheme, user User, modelID string) error {
	...
	mainConfigPath := filepath.Join(home, ".trenchlaw", "config.json")
