...
func (a *App) newUsersPage(schemeName string) tview.Primitive {
	...
	table.SetBorder(true).
		SetTitle(fmt.Sprintf(" [%s::b] USERS · %s ", uiTagRed, schemeName)).
	...
	rebuild := func() {
		...
		for i, u := range users {
			...
			if len(models) > 0 {
				detailText = fmt.Sprintf("  [%s]%d models available[-]", uiTagGreenBold, len(models))
			} else {
				detailText = "  [" + uiTagDanger + "]Inactive / No Access[-]"
			}
			...
			table.SetCell(detailRow, 1,
				tview.NewTableCell("["+uiTagGreen+"]"+u.Type+"  ").
	...
	return a.buildShell(
		"users",
		table,
		" ["+uiTagRed+"]a:[-] add  ["+uiTagRed+"]e:[-] edit  ["+uiTagDanger+"]d:[-] delete  ["+uiTagGreenBold+"]Enter:[-] models  ["+uiTagMuted+"]ESC:[-] back ",
	)
}
...
func (a *App) showUserForm(schemeName string, existing *tuicfg.User, onSave func(tuicfg.User)) {
	...
	form.SetBorder(true).
		...
		SetTitleColor(uiColorAccentRed).
		SetBorderColor(uiColorBorder)
	form.SetBackgroundColor(uiColorPanel)
	form.SetFieldBackgroundColor(uiColorBackground)
	form.SetFieldTextColor(uiColorAccentGreenBold)
	form.SetLabelColor(uiColorText)
	form.SetButtonBackgroundColor(uiColorAccentGreen)
	form.SetButtonTextColor(uiColorInverseText)
