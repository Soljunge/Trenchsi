...
func TestNewJameClientChannel_MissingURL(t *testing.T) {
	_, err := NewJameClientChannel(config.JameClientConfig{}, bus.NewMessageBus())
	if err == nil {
...
func TestNewJameClientChannel_OK(t *testing.T) {
	ch, err := NewJameClientChannel(config.JameClientConfig{
		URL: "ws://localhost:9999/ws",
	}, bus.NewMessageBus())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if ch.Name() != "jame_client" {
		t.Fatalf("name = %q, want jame_client", ch.Name())
	}
}
...
func TestClientChannel_StartTyping(t *testing.T) {
	...
	stop, err := ch.StartTyping(ctx, "jame_client:sess-type")
	if err != nil {
