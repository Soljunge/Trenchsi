...
func (m *Manager) initChannels(channels *config.ChannelsConfig) error {
	...
	if channels.Jame.Enabled && channels.Jame.Token() != "" {
		m.initChannel("jame", "Jame")
	}

	if channels.JameClient.Enabled && channels.JameClient.URL != "" {
		m.initChannel("jame_client", "Jame Client")
	}
