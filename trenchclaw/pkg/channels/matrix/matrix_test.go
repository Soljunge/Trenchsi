...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("trenchlaw")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@trenchlaw hello", want: true},
		{text: "hi @trenchlaw:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0btrenchlaw\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@trenchlaw:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@trenchlaw:matrix.org hello", want: "hello"},
		{in: "@trenchlaw, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@trenchlaw:matrix.org")},
				},
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body: "@trenchlaw:matrix.org hello",
			},
			want: true,
		},
		{
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@trenchlaw:matrix.org">TrenchLaw</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40trenchlaw%3Amatrix.org">TrenchLaw</a> hello`,
			},
			want: true,
		},
	}
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@trenchlaw:matrix.test"), "")
	if err != nil {
