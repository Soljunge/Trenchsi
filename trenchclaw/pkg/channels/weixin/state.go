...
import (
	...
	basechannels "github.com/sipeed/trenchlaw/pkg/channels"
...
func trenchlawHomeDir() string {
	if home := os.Getenv(config.EnvHome); home != "" {
		return home
	}
	userHome, _ := os.UserHomeDir()
	return pkg.ResolveDefaultHome(userHome)
}
...
func buildWeixinSyncBufPath(cfg config.WeixinConfig) string {
	...
	return filepath.Join(trenchlawHomeDir(), "channels", "weixin", "sync", key+".json")
}
