...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via TRENCHLAW_KEY_PASSPHRASE
...
//  2. TRENCHLAW_SSH_KEY_PATH env var
//  3. ~/.ssh/trenchlaw_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "TRENCHLAW_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading TRENCHLAW_KEY_PASSPHRASE from the
...
const SSHKeyPathEnvVar = "TRENCHLAW_SSH_KEY_PATH"

// trenchlawHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const trenchlawHome = "TRENCHLAW_HOME"

const (
	fileScheme = "file://"
	encScheme  = "enc://"
	hkdfInfo   = "trenchlaw-credential-v1"
	saltLen    = 16
	nonceLen   = 12
	keyLen     = 32
)
...
//
// passphrase is required (TRENCHLAW_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// TRENCHLAW_SSH_KEY_PATH env var or ~/.ssh/trenchlaw_ed25519.key.
...
//   - exact match with TRENCHLAW_SSH_KEY_PATH env var
//   - within the TRENCHLAW_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with TRENCHLAW_SSH_KEY_PATH.
	...
	// Within TRENCHLAW_HOME.
	if jameHome := os.Getenv(trenchlawHome); jameHome != "" {
		if isWithinDir(clean, jameHome) {
			return true
		}
	}
...
// Final key: HKDF-SHA256(ikm, salt, info="trenchlaw-credential-v1", 32 bytes)
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set TRENCHLAW_SSH_KEY_PATH or place key at ~/.ssh/trenchlaw_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (TRENCHLAW_SSH_KEY_PATH, TRENCHLAW_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. TRENCHLAW_SSH_KEY_PATH env var
//  3. ~/.ssh/trenchlaw_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the trenchlaw-specific SSH key path if it exists.
