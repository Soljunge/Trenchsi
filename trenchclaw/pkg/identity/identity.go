// Package identity provides unified user identity utilities for TrenchLaw.
...
func MatchAllowed(sender bus.SenderInfo, allowed string) bool {
	...
	// For Telegram, allow bare usernames as a convenience in addition to "@username".
	// Telegram PlatformID is numeric, so a non-numeric plain entry is unambiguous.
	if sender.Platform == "telegram" &&
		!isAtUsername &&
		allowedUser == "" &&
		sender.Username != "" &&
		!isNumeric(trimmed) &&
		sender.Username == trimmed {
		return true
	}
