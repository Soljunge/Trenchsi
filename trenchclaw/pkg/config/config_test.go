...
// TRENCHLAW_SSH_KEY_PATH to its path for the duration of the test. This is required
// whenever a test exercises encryption/decryption via credential.Encrypt or SaveConfig.
func mustSetupSSHKey(t *testing.T) {
	t.Helper()
	keyPath := filepath.Join(t.TempDir(), "trenchlaw_ed25519.key")
	if err := credential.GenerateSSHKey(keyPath); err != nil {
		t.Fatalf("mustSetupSSHKey: %v", err)
	}
	t.Setenv("TRENCHLAW_SSH_KEY_PATH", keyPath)
}
  ...
  "hooks": {
    "processes": {
      "review-gate": {
        "enabled": true,
        "transport": "stdio",
        "command": ["uvx", "trenchlaw-hook-reviewer"],
        "dir": "/tmp/hooks",
        "env": {
          "HOOK_MODE": "rewrite"
        },
...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	cfg := DefaultConfig()
	want := filepath.Join(fakeHome, ".trenchlaw", "workspace")
...
func TestDefaultConfig_WorkspacePath_WithJameclawHome(t *testing.T) {
	t.Setenv("TRENCHLAW_HOME", "/custom/trenchlaw/home")

	cfg := DefaultConfig()
	want := filepath.Join("/custom/trenchlaw/home", "workspace")

	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf("Workspace path with TRENCHLAW_HOME = %q, want %q", cfg.Agents.Defaults.Workspace, want)
	}
}
...
`
	...
	t.Setenv("TRENCHLAW_KEY_PASSPHRASE", "test-passphrase")
	t.Setenv("TRENCHLAW_SSH_KEY_PATH", "")
...
// unchanged when TRENCHLAW_KEY_PASSPHRASE is not set.
func TestLoadConfig_NoSealWithoutPassphrase(t *testing.T) {
	...
	t.Setenv("TRENCHLAW_KEY_PASSPHRASE", "")
	t.Setenv("TRENCHLAW_SSH_KEY_PATH", "")
...
// TestLoadConfig_MixedKeys_NoPassphrase verifies that when TRENCHLAW_KEY_PASSPHRASE
...
func TestDefaultConfig_ReplacesAntigravityWithOpenRouterPreset(t *testing.T) {
	cfg := DefaultConfig()

	var foundOpenRouterPreset bool
	for i := range cfg.ModelList {
		model := cfg.ModelList[i]
		if model.Model == "antigravity/gemini-3-flash" {
			t.Fatal("unexpected antigravity default model in ModelList")
		}
		if model.Model == "openrouter/anthropic/claude-sonnet-4.6" {
			foundOpenRouterPreset = true
			if model.ModelName != "openrouter-claude-sonnet-4.6" {
				t.Fatalf("ModelName = %q, want openrouter-claude-sonnet-4.6", model.ModelName)
			}
			if model.APIBase != "https://openrouter.ai/api/v1" {
				t.Fatalf("APIBase = %q, want https://openrouter.ai/api/v1", model.APIBase)
			}
		}
	}

	if !foundOpenRouterPreset {
		t.Fatal("OpenRouter Claude Sonnet preset not found in ModelList")
	}
}
...
func TestFilterSensitiveData_AllTokenTypes(t *testing.T) {
	...
	cfg.security = &SecurityConfig{
		...
		Channels: &ChannelsSecurity{
			...
			Jame:     &JameSecurity{Token: "jame-token-abc123"},
			IRC: &IRCSecurity{
