...
```bash
chmod 600 ~/.trenchlaw/security.yml
```
...
```bash
cp ~/.trenchlaw/config.json ~/.trenchlaw/config.json.backup
```
...
Edit ~/.trenchlaw/.security.yml and replace placeholders with your actual keys.
...
```bash
rm ~/.trenchlaw/config.json.backup
```
