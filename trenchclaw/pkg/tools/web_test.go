...
// with cf-mitigated: challenge triggers a retry using the honest trenchlaw User-Agent,
...
func TestWebFetchTool_CloudflareChallenge_RetryWithHonestUA(t *testing.T) {
	...
	// Second request must use the honest trenchlaw user agent
	if !strings.Contains(receivedUAs[1], "trenchlaw") {
		t.Errorf("retry request UA = %q, want it to contain 'trenchlaw'", receivedUAs[1])
	}
}
