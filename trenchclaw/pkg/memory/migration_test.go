...
func TestMigrateFromJSON_SkipsMetaJSONFiles(t *testing.T) {
	...
	if addErr := store.AddMessage(ctx, "agent:main:jame:direct:jame:test", "user", "keep me"); addErr != nil {
		t.Fatalf("AddMessage: %v", addErr)
	}
	if summaryErr := store.SetSummary(ctx, "agent:main:jame:direct:jame:test", "keep summary"); summaryErr != nil {
		t.Fatalf("SetSummary: %v", summaryErr)
	}

	metaPath := filepath.Join(sessionsDir, "agent_main_jame_direct_jame_test.meta.json")
	...
	history, err := store.GetHistory(ctx, "agent:main:jame:direct:jame:test")
	if err != nil {
		t.Fatalf("GetHistory: %v", err)
	}
	...
	summary, err := store.GetSummary(ctx, "agent:main:jame:direct:jame:test")
	if err != nil {
		t.Fatalf("GetSummary: %v", err)
	}
